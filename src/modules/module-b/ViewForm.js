...
import 'https://cdn.jsdelivr.net/npm/@capitec/omni-components@esm/dist/email-field/index.js';
...
import 'https://cdn.jsdelivr.net/npm/@capitec/omni-components@esm/dist/pin-field/index.js';
...
<div class="preview">
    <div class="form-container">
        ...
        <omni-button label="Submit" type="primary"></omni-button>
    </div>
</div>`;

export class ViewForm extends HTMLElement {
    constructor() {
        super();
        this.attachShadow({ mode: 'open' });
        this.shadowRoot.appendChild(template.content.cloneNode(true));

        const emailField = this.shadowRoot.querySelector('omni-email-field');
        emailField.addEventListener('input', (e) => this._clearEmailFieldError(e));

        const pinField = this.shadowRoot.querySelector('omni-pin-field');
        pinField.addEventListener('input', (e) => this._clearPinFieldError(e));

        const check = this.shadowRoot.querySelector('omni-check');
        check.addEventListener('click', (e) => this._clearTermsError(e));

        const button = this.shadowRoot.querySelector('omni-button');
        button.addEventListener('click', (e) => this._submitForm(e));

        // this.render();
    }

    _clearEmailFieldError(e) {
        const emailField = e.currentTarget;
        emailField.error = '';
    }

    _clearPinFieldError(e) {
        const pinField = e.currentTarget;
        pinField.error = '';
    }
    ...
    _submitForm() {
        const check = this.shadowRoot?.getElementById('form-check');
        const emailField = this.shadowRoot?.getElementById('form-email-field');
        const pinField = this.shadowRoot?.getElementById('form-pin-field');
    ...
    // render() {
    //     const size = this.getAttribute('size') ?? '#209dee';
...
window.customElements.define('view-form', ViewForm);
